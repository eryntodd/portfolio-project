...
The convention is, filmType is an empty string when no film is loaded, both framesRemaining and totalFrames are zero when the camera has no film. When film is loaded, framesRemaining and totalFrames are both set to the number of frames in the 35mm roll, and framesRemaining decreases by one each time a photo is taken. The value of framesRemaining will never be negative.

The correspondence is that the abstract state of the camera is shown by these three fields. The number of remaining frames corresponds to framesRemaining, the capacity of the film roll corresponds to totalFrames.
